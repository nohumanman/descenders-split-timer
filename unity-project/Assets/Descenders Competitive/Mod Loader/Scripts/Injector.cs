...
namespace SharpMonoInjector
{
    public class Injector : IDisposable
    {
        ...
        private IntPtr GetClassFromName(IntPtr image, string @namespace, string className)
        {
            ...
            ThrowIfNull(@class, mono_class_from_name + " - " + _memory.ReadString(_memory.AllocateAndWrite(@namespace), 256, Encoding.UTF8));
            return @class;
        }
        ...
        {
            int len = _memory.ReadInt((IntPtr)((long)monoString + (Is64Bit ? 0x10 : 0x8)));
            return _memory.ReadUnicodeString((IntPtr)((long)monoString + (Is64Bit ? 0x14 : 0xC)), len * 2);
        }
        ...
        private void RuntimeInvoke(IntPtr method)
        {
            ...
            if (exc != IntPtr.Zero)
            {
                ...
                string message = ReadMonoString((IntPtr)_memory.ReadInt((IntPtr)((long)exc + (Is64Bit ? 0x20 : 0x10))));
        ...
        private IntPtr Execute(IntPtr address, params IntPtr[] args)
        {
            ...
            IntPtr thread = Native.CreateRemoteThread(_handle, IntPtr.Zero, 0, alloc, IntPtr.Zero, 0, out poop);
            ...
            //System.IO.File.AppendAllText("retval.txt", Exports.First(e => e.Value == address).Key + " - Value: " + ((IntPtr)_memory.ReadInt(retValPtr)).ToString() + "\n");
